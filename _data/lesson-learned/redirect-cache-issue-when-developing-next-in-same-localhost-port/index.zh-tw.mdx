...
事情都很順利，不管是在 production 還是 development 你都成功導引訪客到目標網址。然而當你想要開發另外一個 nextjs 專案，在 development 時使用同樣的開發網址http://localhost:3000。你會發現事情怪怪的，即使沒有在 `next.config.js` 指定轉址，它一樣把你轉到了 http://localhost:3000/en
...
那接下來的問題就會是，如果我想要在 production 環境設置 308 Permanent Redirect，但我在本地開發其他專案，想要使用同一個 Port 卻又不想被 cache 影響該怎麼做。

### 解決方法

1. 無差別：每次轉換開發專案時就清理一次 cache（清除瀏覽記錄）
2. 只清理 localhost:3000：在轉換專案時，於 http:localhost:3000 長按重新整理鈕，「選擇清除快取並強制重新載入」（Chrome） 
3. 無差別（不推薦）：Developer tool -> Network -> check disable cache（Chrome）
4. 使用 Browser hard refresh[^8]
