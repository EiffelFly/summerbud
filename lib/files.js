...
export const getLessenLearnedContent = async ({ isPageRoot, lang, slug }) => {
  let fullPath;
  if (isPageRoot) {
    fullPath = path.join(
      DirectoryPath.lessenLearned,
...
export const getLessenLearnedPostsMeta = ({ lang }) => {
  const dir = path.join(DirectoryPath.lessenLearned);
  let llPaths = getAllLessenLearnedFilesames(dir, [], true);
  
  llPaths = llPaths.filter((llpath) => llpath.split("/").length === 4 && llpath.split("/")[3].split(".")[1] === lang);
