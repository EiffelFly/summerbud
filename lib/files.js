...
export const getLessenLearnedContent = async ({ isPage, lang, slug }) => {
  let fullPath;
  if (isPage) {
    fullPath = path.join(
...
export const getAllLessenLearnedPath = () => {
  const dir = path.join(DirectoryPath.lessenLearned);
  const  llPaths = getAllLessenLearnedFilesames(dir, [], false);
  let paths = [];
...
export const getLessenLearnedPostsMeta = ({ lang }) => {
  const dir = path.join(DirectoryPath.lessenLearned);
  let llPaths = getAllLessenLearnedFilesames(dir, [], false);
  llPaths = llPaths.filter((llpath) => llpath.split("/")[1].split(".")[1] === lang);
  const meta = llPaths.map((llPath) => {
    const fullPath = path.join(DirectoryPath.lessenLearned, llPath);
    const slug = llPath.split("/")[0];
    const fullSlug = `/lessen-learned/${slug}`;
    const fileContents = fs.readFileSync(fullPath, "utf-8");
    const { data } = matter(fileContents.trim());
    return {
      path: fullSlug,
...
export const getAllLessenLearnedFilesames = (
  directoryPath,
  filesList,
  havePrefix
) => {
  ...
  files.forEach((file) => {
    if (fs.statSync(`${directoryPath}/${file}`).isDirectory()) {
      filesList = getAllLessenLearnedFilesames(
        `${directoryPath}/${file}`,
        filesList,
        havePrefix
      );
    } else {
      let path = getRemainPathAfterIt("lessen-learned", `${directoryPath}/${file}`);
      if (havePrefix) {
        path = `/lessen-learned/${path}`;
      }
      filesList.push(path);
    }
  });
