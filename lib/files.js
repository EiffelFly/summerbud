...
export function getAllFileNames(directoryPath, filesList = [], prefix) {
  ...
  files.forEach((file) => {
    if (fs.statSync(`${directoryPath}/${file}`).isDirectory()) {
      filesList = getAllFileNames(`${directoryPath}/${file}`, filesList, prefix);
    } else {
      if (prefix){
        filesList.push(path.join("/", prefix, path.basename(directoryPath), "/", file));
      } else {
...
export const getAllPostsFilename = () => {
  ...
  const llFilenames = getAllFileNames(DirectoryPath.lessenLearned, [], "lessen-learned");
...
export const getAllSeriesFileName = (
  ...
  files.forEach((file) => {
    if (fs.statSync(`${directoryPath}/${file}`).isDirectory()) {
      filesList = getAllSeriesFileName(`${directoryPath}/${file}`, filesList, havePrefix);
    } else {
