...
const DirectoryPath = {
  ...
  howTo: path.resolve(process.cwd(), "_data", "how-to"),
  thought: path.resolve(process.cwd(), "_data", "thought"),
};
...
export const getAllPostsFilename = () => {
  ...
  const howToFileNames = getAllFileNames(DirectoryPath.howTo, [], "how-to")
  const thoughtFileNames = getAllFileNames(DirectoryPath.thought, [], "thought")
...
export const getAllFilePath = (type, directoryPath, filesList = []) => {
  if (!directoryPath) {
...
export const constructPathParams = (type) => {
  ...
  paths.forEach((path) => {
    ...
    const slug = pathChunk.pop();

    switch (type) {
      case "series": {
        const seriesSlug = pathChunk[1];
        params.push({
          params: {
            slug: slug,
            lang: lang,
...
export const getAllArticleContent = async ({ type, pathTail }) => {
  const fullPath = path.join(DirectoryPath[`${type}`], pathTail);
  const fileContents = fs.readFileSync(fullPath, "utf-8");
  const { content, data } = matter(fileContents.trim());
  const code = await prepareMDX(content);
  return {
