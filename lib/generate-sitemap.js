...
(async () => {
  ...
  const sitemap = `
    ...
    <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
      ...
      <url>
        <loc>${`https://summerbud.org/zh-tw/about`}</loc>
      </url>
      <url>
        <loc>${`https://summerbud.org/en/about`}</loc>
      </url>
      <url>
        <loc>${`https://summerbud.org/zh-tw/lesson-learned`}</loc>
      </url>
      <url>
        <loc>${`https://summerbud.org/en/lesson-learned`}</loc>
      </url>
      <url>
      ...
      <url>
        <loc>${`https://summerbud.org/zh-tw/series`}</loc>
      </url>
      <url>
        <loc>${`https://summerbud.org/en/series`}</loc>
      </url>
      <url>
        <loc>${`https://summerbud.org/zh-tw/projects`}</loc>
      </url>
      <url>
        <loc>${`https://summerbud.org/en/projects`}</loc>
      </url>
      ${allPages
        .map((page) => {
          return `
            <url>
              <loc>${`https://summerbud.org/${page.lang}/${page.path}`}</loc>
            </url>
          `;
        })
        .join("")}
